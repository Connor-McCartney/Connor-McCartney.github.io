...
MA = block_matrix([
    ...
    [build_submatrix(A10), build_submatrix(A11)], 
])
...
class Kzber:
    #def __init__(self, q = 3329, d = 256, k = 2, B = 2):
    def __init__(self, q = 3329, d = 50, k = 2, B = 2):
        self.q = q
        self.d = d
        self.k = k
        self.B = B
...
e = PKE.e
assert t - A*s == e
print([i if 0<=i<3 else int(i)-q for i in list(e[0][0])])
print([i if 0<=i<3 else int(i)-q for i in list(e[1][0])])
print('---')
...
M = Matrix(t0 + t1).T.augment(MA).augment(diagonal_matrix(2*d*[q])).T.LLL()
